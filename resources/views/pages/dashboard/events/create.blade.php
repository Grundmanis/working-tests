<x-app-layout>
    ...
    <div class="p-6 dark:bg-gray-900 min-h-screen dark:text-gray-200 flex justify-center">
        <div class="w-full max-w-6xl">
            ...
            <form method="POST" action="{{ route('dashboard.event.store') }}" x-data="eventForm()"
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                        class="mt-1 w-full border dark:border-gray-700 rounded-md px-3 py-2 dark:bg-gray-800 dark:text-gray-100">{{ old('description') }}</textarea>
                </div>
                        ...
                        <div class="flex gap-2 mt-1">
                            <select name="location_id"
                                ...
                                @foreach ($locations as $loc)
                                    <option value="{{ $loc->id }}"
                                        @if (old('location_id') == $loc->id) selected @endif>
                                        {{ $loc->name }}
                                    </option>
                                @endforeach
                            </select>
